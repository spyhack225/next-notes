...
| 8. TTS | Apple voice selection and preview, plus opt-in Pocket TTS CoreML playback with four voices, download, model generation and interrupt probe. `docs/tts-benchmark-2026-09-13.md` compares Apple, Kokoro ONNX and Piper. | Listen to the Pocket output and compare its prosody with Apple; measure first acoustic output and sustained playback on the actual device. Kokoro/Piper remain benchmark-only. |
...
| 9–10. TTS and duplex | Apple and Pocket TTS play through a cancellable clause queue. Apple voice picker and optional local Pocket download provide a voice choice. The prior benchmark measured Apple, Kokoro ONNX and Piper; a separate Pocket probe produces a WAV and tests playback interruption. | Listen and compare quality, real acoustic interruption and speaker onset, including Pocket's stream pacing on this M3. |
